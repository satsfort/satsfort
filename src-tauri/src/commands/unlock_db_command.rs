...
pub async fn unlock_db_with_path(
    password: String,
    ...
    let pool = SqlitePool::connect_with(options).await.map_err(|error| {
        log_unlock_failure("connect_with_key", Some(db_path), &error.to_string());
    ...
    sqlx::query("SELECT count(*) FROM sqlite_master")
        ...
        .await
        .map_err(|error| {
            log_unlock_failure("verify_cipher_key", Some(db_path), &error.to_string());
            "Wrong password or database error".to_string()
        })?;

    run_pending_migrations(&pool, db_path).await?;
...
pub async fn unlock_db(password: String, app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    unlock_db_with_path(password, db_path.as_path(), &state).await
}
