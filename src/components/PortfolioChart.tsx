...
export function PortfolioChart({ history, priceUsd, unit }: Props) {
  ...
  const yAt = (btc: number) =>
    PAD.top +
    (1 - (btc - minBtc) / (maxBtc - minBtc)) * (H - PAD.top - PAD.bottom);

  const linePath = points
    .map((p, i) => `${i === 0 ? "M" : "L"}${xAt(i).toFixed(2)},${yAt(p.btc).toFixed(2)}`)
    .join(" ");

  const areaPath =
    `${linePath} L${xAt(points.length - 1).toFixed(2)},${yAt(0).toFixed(2)} ` +
    `L${xAt(0).toFixed(2)},${yAt(0).toFixed(2)} Z`;

  const yTicks = 4;
  const tickValues = Array.from({ length: yTicks + 1 }, (_, i) =>
    (minBtc + (i * (maxBtc - minBtc)) / yTicks)
  );
      ...
      <div className="chart-head">
        <div>
          <div className="muted small">
            {hovered ? formatDate(hovered.date) : "Last 2 years"}
          </div>
