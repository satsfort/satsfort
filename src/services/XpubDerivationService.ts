...
function uint32BEToBytes(value: number): Uint8Array {
 ...
 */
function convertToXpub(extendedKey: string): string {
    ...
    const xpubVersionBytes = uint32BEToBytes(VERSION_BYTES.xpub.public);
    const converted = concatBytes(xpubVersionBytes, decoded.slice(4));
...
/**
 * Derives a P2SH-P2WPKH (Nested SegWit) address from a public key.
 * Addresses start with '3'.
 */
function deriveP2SHAddress(publicKey: Uint8Array): string {
 ...
 */
function deriveP2WPKHAddress(publicKey: Uint8Array): string {
...
/**
 * Derives a P2TR (Taproot) address from a public key.
 * Addresses start with 'bc1p'.
 */
function deriveP2TRAddress(publicKey: Uint8Array): string {
 ...
 */
export function deriveAddressesFromExtendedKey(
    extendedKey: string,
    ...
    const xpub = convertToXpub(extendedKey);
    ...
    const purpose = getBipPurpose(derivationType);
    const addresses: DerivedAddressInfo[] = [];

    for (let i = 0; i < count; i++) {
        const childNode = chainNode.derive(i);
        const address = deriveAddress(childNode.publicKey, derivationType);
 ...
 */
export function validateExtendedKey(extendedKey: string): string | null {
    try {
        const xpub = convertToXpub(extendedKey);
